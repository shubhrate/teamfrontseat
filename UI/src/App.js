...
import './App.css';
import User2Screen from './User2Screen.js';
...
class App extends Component {
  ...
  render() {
    let makeElementForScreen = (screenId, baseProps, atTop, forward) => {
      ...
      switch (screenId) {
        default:
          return null;
        case 'user2':
          return (<User2Screen {...screenProps} />)
        case 'login1':
    ...
    return (
      <div className="App">
        <Switch>
          <Route path="/" render={(props) => makeElementForScreen('launch', props.location.state, true, true)} exact />
          <Route path="/user2" render={(props) => {
            return makeElementForScreen('user2', props.location.state, true, true);
          }} />
          <Route path="/login1" render={(props) => {
            return makeElementForScreen('login1', props.location.state, true, true);
          }} />
          <Route path="/user" render={(props) => {
            return makeElementForScreen('user', props.location.state, true, true);
          }} />
          <Route path="/guide" render={(props) => {
            return makeElementForScreen('guide', props.location.state, true, true);
          }} />
          <Route path="/launch" render={(props) => {
            return makeElementForScreen('launch', props.location.state, true, true);
          }} />
        </Switch>
      </div>
    );
